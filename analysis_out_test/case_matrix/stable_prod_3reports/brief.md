...
report_count: 3
stable_findings_count: 21
tuning_recommendations_count: 16
ephemeral_findings_count: 5

## Reports
- prod1: pgprofile_srv=10_3_81_94_from=2026_07_31_09_00_to=2026_08_01_00.html
  server: tsldd-pprb01138.delta.sbrf.ru
  interval: 2026-07-31 09:00:02+03 .. 2026-08-01 00:00:02+03 (15.0 h)
  findings_in_report: 70
- prod2: pgprofile_srv=10_3_81_94_from=2026_08_11_16_30_to=2026_08_12_09.html
  server: tsldd-pprb01138.delta.sbrf.ru
  interval: 2026-08-11 16:30:02+03 .. 2026-08-12 09:00:02+03 (16.5 h)
  findings_in_report: 75
- prod3: pgprofile_srv=10_3_81_94_from=2026_08_12_16_00_to=2026_08_13_08.html
  server: tsldd-pprb01138.delta.sbrf.ru
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: cautious | change_impact: medium
- example: statecontracts: idle_in_transaction_time=1416504.9s (threshold 3600s), idle_in_transaction_session_timeout=0
- problem: Долгие idle in transaction сессии держат snapshots, блокируют vacuum
- GUC `idle_in_transaction_session_timeout` → set_nonzero [safety=cautious, impact=medium] current: prod1=0, prod2=0, prod3=0
...
- example: postgres.pgse_profile.sample_statements: table hit_pct=82.71% (threshold 95.0%)
- problem: Горячие таблицы читаются с диска чаще ожидаемого — узкий buffer pool,
- GUC `shared_buffers` → increase [safety=restart_required, impact=high] current: prod1=504320, prod2=504320, prod3=537600
...
- GUC `effective_cache_size` → review_increase [safety=safe, impact=low] current: prod1=1512960, prod2=1512960, prod3=1512960
...
- GUC `work_mem` → review_decrease_or_role_level [safety=risky, impact=high] current: prod1=16384, prod2=16384, prod3=16384
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: restart_required | change_impact: high
- example: Checkpoint write time: 25187.0s over 15.0h interval (threshold 300s)
- problem: Длительная запись checkpoint увеличивает IO latency и может влиять на
- GUC `checkpoint_completion_target` → review_increase [safety=safe, impact=low] current: prod1=0.5, prod2=0.7, prod3=0.7
...
- GUC `shared_buffers` → review_increase [safety=restart_required, impact=high] current: prod1=504320, prod2=504320, prod3=537600
...
- example: statecontracts/as_admin: max=37443.1ms, calls=1
...
- GUC `work_mem` → review_increase_role_level [safety=risky, impact=high] current: prod1=16384, prod2=16384, prod3=16384
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: cautious | change_impact: medium
- example: Requested checkpoints: 56/58 (96.6%), threshold 30%
- problem: Большая доля requested checkpoints означает, что WAL заполняется быстрее,
- GUC `max_wal_size` → increase [safety=cautious, impact=medium] current: prod1=8192, prod2=12288, prod3=12288
...
- example: postgres: connection pressure sessions=2924 (532% of max_connections=550)
- problem: Высокая утилизация соединений или много idle-сессий — риск исчерпания
- GUC `max_connections` → review_decrease_with_pooler [safety=cautious, impact=high] current: prod1=550, prod2=550, prod3=550
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: cautious | change_impact: medium
- example: wal_buffers_full: 932205 (threshold 1000)
- problem: wal_buffers переполняется — процессы ждут освобождения WAL buffer.
- GUC `wal_buffers` → increase [safety=cautious, impact=medium] current: prod1=2048, prod2=2048, prod3=2048
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: restart_required | change_impact: high
- example: Backend buffers written (21559495) exceed checkpoint buffers written (16793607)
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: cautious | change_impact: medium
- example: postgres.pg_catalog.pg_db_role_setting: seq_scan=2859668, idx_scan=133148, ratio=21.5
- problem: Много seq scan при низком idx_scan — возможно отсутствует индекс
- GUC `random_page_cost` → review_decrease_ssd [safety=cautious, impact=medium] current: prod1=2, prod2=2, prod3=2
...
- GUC `effective_io_concurrency` → increase [safety=cautious, impact=low] current: prod1=300, prod2=300, prod3=300
...
- example: statecontracts: blk_read_time=248.3s (threshold 60s)
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: safe | change_impact: low
- example: statecontracts: dominates statement time 98.8% (total_exec_time=47626.27)
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: safe | change_impact: low
- example: statecontracts.pg_toast.pg_toast_76725: heap_blks_read=4688370
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: safe | change_impact: low
- example: statecontracts.statecontracts.t_dspc_sys_config: vacuum_ops=139, analyze_ops=139
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: safe | change_impact: low
- example: statecontracts/as_admin: wal=105.8GB, shared_blks_dirtied=5782196
...
- GUC `statement_timeout` → set_nonzero [safety=safe, impact=low] current: prod1=0, prod2=0, prod3=0
...
- GUC `lock_timeout` → set_nonzero [safety=safe, impact=low] current: prod1=0, prod2=0, prod3=0
...
- stability: 3/3 (100%) in prod1, prod2, prod3
- change_safety: safe | change_impact: low
- example: Unused index statecontracts.statecontracts.t_participant.i_participant_aggregateroot_id: size=1026 MB
...
### In some reports (not all)
- [warning] autovacuum.table_high_dead_pct: 2/3 — prod1, prod2
- [warning] autovacuum.table_many_dead_tuples: 2/3 — prod1, prod3
- [warning] io.table_growth: 2/3 — prod2, prod3
### Only in one report
#### prod1
- [warning] memory.huge_pages_recommended: huge_pages=try with shared_buffers=3940MB (≥2048MB): prefer huge_pages=on; shared_memory_size_in_huge_pages=2226
#### prod2
